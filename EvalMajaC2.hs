...
data ExpResult = MajaInt Integer | MajaBool Bool | MajaVoid | MajaLoc [Loc] 
               | MajaArray [Loc] | MajaList [ExpResult] 
...
type SEnv = M.Map Var (M.Map Var Type)                                                 
...
assignFunParams d venv p = assign' d p venv
   where
      ...
      assign' aaa@((DVar t v):ds) (p:ps) venv = do
         case t of
            TRef _ -> do
               loc <- getLocFromExp p
...
fromLocToEval loc = do
   ...
   case val of
      MajaArray locs -> mapM fromLocToEval locs >>= return . MajaList
      MajaStruct var m -> do
...
convertExpResult def elem = convert (def, elem)
   where
      convert (d, elem) = case d of
         MajaList dl -> MajaList $ map convert $ zip dl l
         MajaStructEval var m -> MajaStructEval var $ M.fromList $ zip (M.keys m) $ map convert $ zip (M.elems m) l
         MajaTupleEval dl -> MajaTupleEval $ map convert $ zip dl l 
         _ -> elem
         where
            MajaList l = elem
...
defaultValue t e = case t of
   ...
   TStruct (TagType var) -> do
      ...
      def <- mapM (\t -> defaultValue t e) (M.elems m)
...
--assign :: MonadState S m => Var -> ExpResult -> m ()
assign v val = do
   var <- getVar v
   case var of
      MajaArray _ -> do
      ...
      MajaStruct var' m -> do
         ...
         l' <- listToStruct list (M.elems m)
         let str = MajaStructEval var' $ M.fromList $ zip (M.keys m) l'
         l <- alloc str
         copy v l
      MajaTuple _ -> do
         let MajaList list = val
         let tup = MajaTupleEval list
         l <- alloc tup
         copy v l
      _ -> do 
         l <- alloc val
...
evalExpM (EITup (ITup e1 e2)) = evalList (e1:e2)
...
--jakies checkForArraysInStruct, zeby miec nie [], ale to potem
   x <- evalExpM e
   newVar t v []
   assign v x
...
execDeclM (DeclA (DArr typ@(TArray t) v arrm)) = do                                 
   ...
   where
      f (MajaInt i) = if i > 0 then fromInteger i
         else error $ "Array length must be greater then 0!"
...
execDeclM (DeclS (Tag var sdecl)) = do                                          
   S (fenv, venv, senv, (store, loc)) <- get
   put $ S (fenv, venv, (M.insert var (M.fromList (map structFieldsToMap sdecl)) senv), (store, loc))

structFieldsToMap (StrField t v) = (v, t)
structFieldsToMap (StrFieldArr t v e) = (v, t)
